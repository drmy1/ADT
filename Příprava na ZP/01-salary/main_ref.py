...
def load_data_file(filepath: str, wanted_year: int):
    ...
    with open(filepath, "r", encoding="utf8") as fd:
        ...
        for line in fd:
            ...
            if len(fields) != 15:
                print(
                    f"V řádku je nevalidní záznam -- špatný počet polí\n{line}\n", len(fields), file=sys.stderr)
                continue
            ...
            if not year_s.isnumeric():
                print(
                    f"V řádku je nevalidní záznam -- Očekávám číselnou hodnotu\n{line}\n{year_s}", file=sys.stderr)
                continue
...
def main(input_path: str):
    ...
    print(
        f"Prumerna mzda se pro rok 2014 a 2015 změnila z {avg14} na {avg15} to je o {avg15-avg14} Kč")
