...
def find_path_dijkstra(graph: Graph, start_id: int, end_id: int, paint=False, verbose=1):
    ...
    # starší verze adthelpers painter počítá s Node -- novější umí vizualizovat i int
    closed: set[int] = set()
    ...
    painter = None
    if paint:
        painter = Painter(graph, priority_queue, closed,
                          None, distances=distances)
...
def main():
    ...
    paint = True
    predecessors, distances = find_path_dijkstra(
        graph, start, end, paint=paint)
    ...
    paint = False
    predecessors, distances = find_path_dijkstra(
        graph, start, end, paint=paint, verbose=0)
